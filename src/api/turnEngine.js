...
const normalizePlayerState = (player, index = 0) => {
...
const advanceTurnState = (gameInput, payload = {}) => {
  ...
  phases.push({
    ...
    deltas: [
      ...
      { metric: 'income', before: 0, after: income, delta: income },
      { metric: 'costs', before: 0, after: roundCurrency(taxes + recurringCosts + debtMinimums), delta: roundCurrency(-(taxes + recurringCosts + debtMinimums)) },
      { metric: 'debtInterest', before: 0, after: debtInterest, delta: roundCurrency(-debtInterest) },
  ...
  if (eventOutcome.kind === 'intended') {
    intendedOutcomes.push({ source: chosenEvent.label, description: eventOutcome.message, explanation: eventOutcome.explanation })
  } else {
    unintendedOutcomes.push({ source: chosenEvent.label, description: eventOutcome.message, explanation: eventOutcome.explanation })
  }
  phases.push({
    id: 'event',
  ...
  intendedOutcomes.push({ source: actionDefinition.label, description: actionResult.message })
  workingPlayer.netWorth = calculateNetWorth(workingPlayer)
...
const beginNextTurnState = (gameInput) =>
  normalizeGameState({
    ...normalizeGameState(gameInput),
    status: 'turn_ready',
    pendingHandoff: null,
