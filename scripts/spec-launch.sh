...
usage() {
  cat <<'USAGE'
Usage:
  bash scripts/spec-launch.sh [SPEC_ID] [launch|pre|finalize] [--profile <name>] [--no-yolo]
...
Notes:
  ...
  - Use `--profile <profile>` to launch codex with `-p <profile>`.
  - Use `--no-yolo` to omit `--yolo`.
  - `CODEX_PROFILE` and `CODEX_YOLO` still work as fallbacks.
USAGE
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --profile=*)
      cli_profile="${1#*=}"
      [[ -n "$cli_profile" ]] || fail "Missing value for --profile"
      ;;
    --no-yolo)
      cli_yolo="0"
      ;;
...
case "$spec_id" in
  ...
  W2-S1) spec_rel="specs/wave-2/W2-S1-app-flow-integration.md" ;;
  W2-S2) spec_rel="specs/wave-2/W2-S2-storage-resume-integration.md" ;;
  W3-S1) spec_rel="specs/wave-3/W3-S1-vitest-wizard-coverage.md" ;;
  W3-S2) spec_rel="specs/wave-3/W3-S2-playwright-workflow-coverage.md" ;;
...
export CODEX_PROFILE="${cli_profile:-${CODEX_PROFILE:-}}"
export CODEX_YOLO="${cli_yolo:-${CODEX_YOLO:-1}}"
...
case "$mode" in
  ...
  launch)
    ...
    command -v "$codex_bin" >/dev/null 2>&1 || fail "$codex_bin is not installed or not on PATH"
    codex_cmd=("$codex_bin")
    if [[ -n "$CODEX_PROFILE" ]]; then
      codex_cmd+=("-p" "$CODEX_PROFILE")
    fi
    if [[ "$CODEX_YOLO" != "0" ]]; then
      codex_cmd+=("--yolo")
    fi
    run_dispatch --phase pre
    ...
    cd "$worktree_root"
    echo "[spec-launch] Codex command: ${codex_cmd[*]}"
